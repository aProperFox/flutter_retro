...
IconData getIconFromName(String name) {
  try {
    ...
    try {
      ...
      throw Exception("Invalid icon name \"$name\"");
    }
  }
}

String getNameFromIcon(IconData icon) {
  if (IconsMap.containsValue(icon)) {
    return IconsMap.keys.firstWhere((key) => IconsMap[key] == icon);
  } else if (FontAwesomeIconsMap.containsValue(icon)) {
    return FontAwesomeIconsMap.keys
        .firstWhere((key) => FontAwesomeIconsMap[key] == icon);
  } else {
    throw Exception("Unknown icon \"$icon\". Is it a custom icon?");
  }
}
